...
// reinicio de formulario al recargar la pagina
document.addEventListener("DOMContentLoaded", function () {
    document.getElementById("formulario").reset();
});
// toggle input para check boxes
...
regionSelect.addEventListener("change", function () {
    ...
    let selectedRegion = region_comuna.regiones.find(r => r.numero == this.value);
    if (selectedRegion) {
...
// max de 5 check boxes
document.addEventListener("DOMContentLoaded", function () {
    const checkboxes = document.querySelectorAll('input[name="redsocial"]');
    
    checkboxes.forEach(checkbox => {
        checkbox.addEventListener("change", function () {
            const selected = document.querySelectorAll('input[name="redsocial"]:checked');
...
window.onload = function() {
    ...
    document.getElementById('fecha-hora-termino').value = ahora.toISOString().slice(0, 16);

    ahora.setHours(ahora.getHours() - 3);
    document.getElementById('fecha-hora-inicio').value = ahora.toISOString().slice(0, 16);
};
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function toggleOtroTema() {
        if (temaSelect.value === "otro") {
            otroTemaInput.style.display = "inline-block";
        } else {
            otroTemaInput.style.display = "none";
            otroTemaInput.value = ""; // Limpiar el campo cuando no se usa
        }
    }

    temaSelect.addEventListener("change", toggleOtroTema);
    toggleOtroTema(); // Ejecutar al inicio para el estado inicial
});

document.addEventListener("DOMContentLoaded", function () {
    const contenedorImagenes = document.getElementById("contenedor-imagenes");
    const botonAgregar = document.getElementById("agregar-imagen");
    const errorImagenes = document.getElementById("error-imagenes");
    const formulario = document.getElementById("formulario");

    function contarInputs() {
        return contenedorImagenes.getElementsByTagName("input").length;
    }

    function validarImagenes() {
        const totalInputs = contarInputs();
        if (totalInputs < 1 || totalInputs > 5) {
    ...
    formulario.addEventListener("submit", function (event) {
        if (!validarImagenes()) {
            event.preventDefault(); // Evita el envío si no cumple con la condición
        }
    });
});
